...
**
**  Brief converting to C language using original code
**  Converted by: Hyungjin Cha
**
**  6 Nov 2021
**
**************************************************************************/
...
typedef enum { false, true } _svd_bool;
...
inline void condSwap(_svd_bool c, float* X, float* Y)
{
    // used in step 2
    float Z = *X;
    *X = c ? *Y : *X;
...
inline void condNegSwap(_svd_bool c, float* X, float* Y)
{
...
inline void approximateGivensQuaternion(float a11, float a12, float a22, float* ch, float* sh)
{
    ...
    *ch = 2 * (a11 - a22);
    *sh = a12;
    _svd_bool b = _gamma * (*sh) * (*sh) < (*ch) * (*ch);
